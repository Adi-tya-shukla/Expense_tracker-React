  ...
  return async (dispatch) => {
    const fetchData = async () => {
      const response = await fetch(
        "https://expense-tracker-a7f26-default-rtdb.firebaseio.com/expenses.json"
      );
      if (!response.ok) {
...
export const editHandler=(id)=>{
  ...
  return async(dispatch)=>{
    const editItem = async() => {
          const response = await axios.get(`https://expense-tracker-a7f26-default-rtdb.firebaseio.com/expenses/${id}.json`)
