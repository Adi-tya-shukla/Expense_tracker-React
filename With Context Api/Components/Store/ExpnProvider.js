...
import ExpnContext from './ExpnContext';

const ExpnProvider = (props) => {
    ...
    if (token) {
      try {
        const res = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyDBUXC46Nhb92NFzchS0XTtwXge3LCkC80",
          {
            method: "POST",
            headers: {
    ...
    if (name && profile) {
      try {
        const response = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyDBUXC46Nhb92NFzchS0XTtwXge3LCkC80",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
const verifyEmailHandler = async () => {
  try {
    ...
    const response = await axios.post(
      "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyDBUXC46Nhb92NFzchS0XTtwXge3LCkC80",
      {
        idToken: token,
...
const logInHandler = (token, mail) => {
  ...
  return (
    <ExpnContext.Provider value={cartContext}>
      {props.children}
    </ExpnContext.Provider>
  );
};

export default ExpnProvider;